...
describe('Other in Collection.js', () => {
  ...
  describe('randomId', () => {
    ...
    it('returns the right length', () => {
      expect(Collection.randomId(5).length).toBe(5);
    });

    it('uses a default when no length given', () => {
      const result = Collection.randomId();
      expect(typeof result).toBe('string');
      expect(result.length).toBeGreaterThan(0);
    });

    it('returns seemingly random results', () => {
      expect(Collection.randomId(5)).not.toBe(Collection.randomId(5));
    });
  ...
  describe('CRUD', () => {
    ...
    describe('_insert', () => {
      ...
      it('throws on no _id', () => {
        ...
        expect(() => { col._insert(docWithoutId)}).toThrow();
      });

      it('persists on match (only)', () => {
        const db = { idb: { put: jest.fn() }};
        const col = new Collection(db, 'test');
        const doc = { _id: 1 };
      ...
      it('calls sendChanges() with insert, strId and fulldoc', () => {
        ...
        expect(col.sendChanges.mock.calls[0][0]).toBe('insert');
    ...
    describe('insert', () => {
      ...
      beforeAll(() => {
        realRandomId = Collection.randomId;
        Collection.randomId = () => 'randomId';
      });

      afterAll(() => {
        Collection.randomId = realRandom;
      });
      ...
      it('adds an id if none given (non-local collection)', () => {
        let col = new Collection(FakeDb, 'test');
        col._insert = jest.fn();
        col._didUpdate = () => {};

        col.insert({});
        expect(col._insert.mock.calls[0][0]).toHaveProperty('_id', 'randomId');
      });

      it('adds an id if none given (local collection)', () => {
        let col = new Collection(FakeDb, 'test', { isLocalCollection: true });
        col._insert = jest.fn();
        col._didUpdate = () => {};
    ...
    describe('_update', () => {
      ...
      it('calls sendChanges() with update, strId and ??', () => {
        ...
        col.sendChanges = jest.fn();
        col._update(oldDoc._id, newDoc);
        expect(col.sendChanges.mock.calls[0][0]).toBe('update');
        expect(col.sendChanges.mock.calls[0][1]).toBe('1');
        expect(col.sendChanges.mock.calls[0][2]).toEqual({
          fullDocument: newDoc
        });
    ...
    describe('updateId', () => {
      ...
      it('throws on non-existing doc._id', () => {
        const col = new Collection(FakeDb, 'test');
        expect(() => col.updateId('nonExistant', {})).toThrow('called with id with no match');
      });
      ...
      it('throws on given id of a pendingDelete', () => {
        ...
        expect(() => col.updateId('a', {})).toThrow('called with id with no match');
      });
    ...
    describe('_remove', () => {
      ...
      it('calls sendChanges() with delete, and TODO', () => {
        ...
        expect(col.sendChanges.mock.calls[0][0]).toBe('delete');
        /* TODO
        expect(col.sendChanges.mock.calls[0][1]).toBe('1');
        expect(col.sendChanges.mock.calls[0][2]).toEqual({
          fullDocument: newDoc
