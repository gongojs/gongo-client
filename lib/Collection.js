...
class Collection {
  ...
  insert(document) {
    const docToInsert = this.isLocalCollection ? {
      ...
      _id: document._id || Collection.randomId(),
        ...document,
  ...
  // untested
  watchOne(query, onChange) {
    const cursor = this.find(query).limit(1);
    const singleOnChange = data => onChange(data[0]);
    return cursor.watch(singleOnChange)[0];
  }
...
Collection.randomId = utils.randomId;
