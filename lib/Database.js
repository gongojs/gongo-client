...
class Database {
  ...
  getSubscriptions(includeInactive=false, returnArray=false) {
    const subs = {};
    this.subscriptions.forEach((sub, hash) => {
      if (includeInactive && sub.active === false)
        return;
      subs[hash] = sub.toObject();
    });
    return returnArray ? Object.values(subs) : subs;
  }
  ...
  populateSubscriptions() {
    ...
    Object.entries(subStore.subscriptions).forEach(([hash, subObj]) => {
      ...
      if (!sub) {
        ...
        sub = new Subscription(this, name, opts);
        sub.active = false;
        this.subscriptions.set(hash, sub);
      }
  ...
  processSubResults(subs) {
    for (let sub of subs) {
      ...
      if (sub.error) {
        console.warn("Ignoring subscription", sub);
        continue;
      }
