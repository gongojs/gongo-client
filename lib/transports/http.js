class HTTPTransport {
  ...
  async poll() {
    ...
    const subscriptions = this.db.subscriptions.length
      ? this.db.subscriptions.map(sub => {
        const out = [sub.name];
        if (sub.opts) out[1] = sub.opts;
        return out;
      })
      : undefined;

    const data = { changeSet, subscriptions };
    ...
    if (json.subs)
      this.db.processSubResults(json.subs);
