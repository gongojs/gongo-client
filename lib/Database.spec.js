...
describe('objectifyStringIDs', () => {
  ...
  it('works with ARSON', () => {
    const strId = "54495ad94c934721ede76d90";
    const doc = { _id: strId, __ObjectIDs: ['_id'] };
    objectifyStringIDs(doc);
    const encoded = ARSON.encode(doc);
    const decoded = ARSON.decode(encoded);
    const _id = decoded._id;
    expect(_id._bsontype).toBe('ObjectID');
    const buf = new Buffer(strId.split('').map(c => c.charCodeAt(0)));
    buf.toString('binary')//?
    decoded//?
    expect(_id.id.toString('hex')).toBe(buf.toString());
  });
