...
class GongoIDB {
  ...
  async _putAll() {
    ...
    let size = 0, i = 0;
    for (let arr of Object.values(localToSync))
      size += arr.length + 1; // +1 for tx.done
  ...
  put(collectionName, document) {
    this.toSync[collectionName] = this.toSync[collectionName] || [];
    if (!this.toSync[collectionName].includes(document))
      this.toSync[collectionName].push(document);
